...
(
  ...
  gh loguser
  gh workflow run "var-github".yml
  gh run list --workflow="var-github".yml
  gh run watch -i 1 2>/dev/null
  run_info="$( gh run list --workflow=var-github.yml --limit 1 | grep -v '^STATUS' | grep workflow_dispatch )"
