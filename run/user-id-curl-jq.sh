...
(
  ...
  gh loguser
  gh workflow run "user-id-curl-jq".yml
  gh run list --workflow="user-id-curl-jq".yml
  gh run watch -i 1 2>/dev/null
  run_info="$( gh run list --workflow=user-id-curl-jq.yml --limit 1 | grep -v '^STATUS' | grep workflow_dispatch )"
